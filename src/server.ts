...
const DEFAULT_TIMEOUT_MS = Math.ceil(
	((2 * 1024 * 1024 * 1024) / (50_000_000 / 8)) * 2 * 1000,
);
		...
		try {
			...
			const cleanTempNow = async () => {
				if (tempForCleanup && !tempCleaned) {
					try {
						await tempForCleanup.cleanFileOnTmpFolder();
					} catch (_) {}
					tempCleaned = true;
				}
			};
			...
			if (contentType.includes("multipart/form-data")) {
				...
				try {
					...
					const finalUploadUrlForMultipart =
						uploadUrlParam || uploadUrlFromForm;
					if (!finalUploadUrlForMultipart && !wantsDownload) {
						await cleanTempNow();
						return badRequest(
							"When not using ?download you must provide uploadUrl as a query parameter or form field",
						);
					}

					const handlerResult = await withTemp<{
						type: string;
						outputPath?: string;
						originalName?: string;
						uploadResp?: {
						...
						async (temp) => {
							...
							await streamWebToFile(
								fileStream as ReadableStream<Uint8Array>,
								inputPath,
							);

							originalName =
								(file as File & { name?: string }).name || `converted.${to}`;
							...
							try {
								...
								const res = await withTimeout(async (signal) => {
									return await convertFile(inputPath, fromExt, toExt, {
										temp,
										keepTemp: true,
							...
							if (
								...
								typeof (result as { outputPath?: unknown }).outputPath ===
									"string"
							) {
								const outputPath = (result as { outputPath: string })
									.outputPath;
								...
								try {
									const uploadResult = await withTimeout(async (signal) => {
										...
										const uploadResp = await fetch(
											finalUploadUrlForMultipart as string,
											{
												method: "PUT",
												headers: {
													"Content-Type":
					...
					if (
						handlerResult &&
						handlerResult.type === "stream" &&
						handlerResult.outputPath
					) {
						...
						rs.on("end", async () => {
							try {
								await handlerResult.temp?.cleanFileOnTmpFolder();
							} catch (_) {}
						});
						rs.on("error", async () => {
							try {
						...
						const disposition = buildContentDispositionAttachment(
							originalName,
							String(to),
						);
						...
						return new Response(webStream as unknown as BodyInit, {
							...
							headers: {
								"Content-Type":
									to === "pdf" ? "application/pdf" : "application/octet-stream",
					...
					if (
						handlerResult &&
						handlerResult.type === "upload" &&
						handlerResult.uploadResp
					) {
						return new Response(handlerResult.uploadResp.body, {
							status: handlerResult.uploadResp.status,
							headers: handlerResult.uploadResp.headers,
						});
					}
				} catch (err: unknown) {
					await cleanTempNow();
					...
					return new Response(
						JSON.stringify({
							error: "Failed to stream uploaded file to temp",
							details: msg,
						}),
				...
				const downloadUrl =
					typeof d["downloadUrl"] === "string"
						...
						: undefined;
				from =
					typeof d["from"] === "string" ? (d["from"] as string) : undefined;
				to = typeof d["to"] === "string" ? (d["to"] as string) : undefined;
				uploadUrlFromJson =
					typeof d["uploadUrl"] === "string"
				...
				try {
					const handlerResult = await withTemp<{
						...
						async (temp) => {
							...
							await withTimeout(async (signal) => {
								...
								if (!resp.ok) {
									throw new Error(
										`FETCH_NOT_OK:${resp.status}:${resp.statusText}`,
									);
								}
					...
					return new Response(
						JSON.stringify({
							error: "Failed to fetch/stream downloadUrl",
							details: msg,
						}),
						{ status: 400, headers: { "Content-Type": "application/json" } },
					);
				...
				return jsonResponse(
					{
						...
						details: (result as { stderr?: string }).stderr,
					},
					500,
				);
			}
			...
			if (
				...
				const finalUploadUrl =
					uploadUrlParam || uploadUrlFromForm || uploadUrlFromJson;
				if (!finalUploadUrl) {
					await cleanTempNow();
					return badRequest(
						"When not using ?download you must provide uploadUrl as a query parameter, form field, or in the JSON body",
					);
				}
				...
				return new Response(respArrayBuffer, {
					status: uploadResp.status,
					headers: forwardedHeaders,
				});
			}
			...
			if (wantsDownload) {
				const body = converted.buffer.slice(
			...
			const bodyForUpload = converted.buffer.slice(
				converted.byteOffset,
				converted.byteOffset + converted.byteLength,
			);
			const contentTypeForConverted =
